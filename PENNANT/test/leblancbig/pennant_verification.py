...
print("OMP_NUM_THREADS=32 GOMP_CPU_AFFINITY=\"0 20 1 21 2 22 3 23 4 24 5 25 6 26 7 27 10 30 11 31 12 32 13 33 14 34 15 35 16 36 17 37\" LD_LIBRARY_PATH=%s/src /usr/bin/time -f \"Elapsed Time , %%e, system, %%S, user, %%U, memory, %%M\" %s/bin/hpcrun -o test1_output_folder -e WP_FALSE_SHARING -e MEM_UOPS_RETIRED:ALL_LOADS@500000 -e MEM_UOPS_RETIRED:ALL_STORES@500000 ../../build/pennant leblancbig.pnt" % (adamant_path, ComMon_path))

os.system("OMP_NUM_THREADS=32 GOMP_CPU_AFFINITY=\"0 20 1 21 2 22 3 23 4 24 5 25 6 26 7 27 10 30 11 31 12 32 13 33 14 34 15 35 16 36 17 37\" LD_LIBRARY_PATH=%s/src /usr/bin/time -f \"Elapsed Time , %%e, system, %%S, user, %%U, memory, %%M\" %s/bin/hpcrun -o test1_output_folder -e WP_FALSE_SHARING -e MEM_UOPS_RETIRED:ALL_LOADS@500000 -e MEM_UOPS_RETIRED:ALL_STORES@500000 ../../build/pennant leblancbig.pnt" % (adamant_path, ComMon_path))
...
plt.xticks(np.arange(x1+0.5, x2+0.5, 5))
plt.yticks(np.arange(y1+0.5, y2+0.5, 5)) 
